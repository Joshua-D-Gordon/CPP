...
SmartTeam::SmartTeam(Character* theLeader): leader(theLeader){
...
void SmartTeam::attack(Team* otherTeam){
    ...
    for(Character* c : teamCharts){
        c->dtv=c->distance(victim);
    }
    this->reorderSet();
...
void SmartTeam::changeLeader(Character* leader){
...
Character* SmartTeam::chooseVictim(Team* otherTeam){
    double dis = -1;
...
Character* SmartTeam::chooseVictim(SmartTeam* otherTeam){
    double dis = -1;
    Character* firstelement = nullptr;
